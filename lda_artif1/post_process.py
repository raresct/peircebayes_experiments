...
def post_process():
    ...
    plt.plot(x, avg_lls, linestyle='-.', color='b', label='PB-UGS')
    ...
    pattern2 = re.compile('lls_cgs_([0-9]+)_[0-9]+\.npz')
    files = [join(data_dir,f)  
        for f in listdir(data_dir) 
    ...
    lls_sem = 1.96*stats.sem(all_lls)
    x = np.arange(all_lls.shape[1])
    plt.plot(x, avg_lls, linestyle='--', color='g', label='PB-CGS')
    ...
    plt.plot(x_gibbs, avg_gibbs_lls, linestyle='-', color='r', label='LDA-CGS')
