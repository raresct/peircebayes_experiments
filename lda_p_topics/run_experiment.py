...
def run_experiment():
    ...
    D = 1000
    W = 25
    for j,seed in enumerate(np.random.choice(5000, 4, replace=False)+1):
        print(j)
        
        metric_pb = do_pb(data_dir, j, seed, topics, alpha, beta, D, W, K)
        #print metric_pb
        
        metric_stan = do_stan(data_dir, j, seed, topics, alpha, beta, D, W, K)
        #print metric_stan
        
        metric_prism = do_prism(data_dir, j, seed, topics, alpha, beta, D, W, K)
        #print metric_prism

        metric_tm_gibbs = do_tm_gibbs(data_dir, j, seed, topics, alpha, beta, D, W, K)
        #print metric_tm_gibbs

        metric_tm_vem = do_tm_vem(data_dir, j, seed, topics, alpha, beta, D, W, K)
        print metric_tm_vem
...
def do_tm_gibbs(data_dir, j, seed,  topics, alpha, beta, D, W, K):
